...
def get_image_data(filename, subfolder, image_type):
    ...
    try:
        ...
        print(f"worker-comfyui - Error fetching media data for {filename}: {exc}")
        return None
    except Exception as exc:
        print(
            f"worker-comfyui - Unexpected error fetching media data for {filename}: {exc}"
        )
        return None


def snapshot_output_videos():
    """Return video paths and signatures so SaveVideo files can be found reliably."""
    snapshot = {}
...
def encode_media_output(job_id, filename, media_bytes, errors):
    """Encode a ComfyUI output as an S3 URL or a Base64 response item."""
    ...
    if os.environ.get("BUCKET_ENDPOINT_URL"):
        ...
        try:
            ...
            print(f"worker-comfyui - Uploading media {filename} to S3...")
            s3_url = rp_upload.upload_image(job_id, temp_file_path)
            return {"filename": filename, "type": "s3_url", "data": s3_url}
        except Exception as exc:
            error_msg = f"Error uploading {filename} to S3: {exc}"
            print(f"worker-comfyui - {error_msg}")
            ...
            if temp_file_path and os.path.exists(temp_file_path):
                try:
                    ...
                    print(f"worker-comfyui - Error removing {temp_file_path}: {exc}")
    try:
        encoded = base64.b64encode(media_bytes).decode("utf-8")
        print(f"worker-comfyui - Encoded media {filename} as base64")
        return {"filename": filename, "type": "base64", "data": encoded}
    except Exception as exc:
        error_msg = f"Error encoding {filename} to base64: {exc}"
        print(f"worker-comfyui - {error_msg}")
        errors.append(error_msg)
        return None
...
def handler(job):
    ...
    video_data = []
    errors = []
    videos_before = snapshot_output_videos()
    ...
    try:
        ...
        for node_id, node_output in outputs.items():
            ...
            for media_key in ("videos", "video", "gifs"):
                ...
                for media_info in media_items:
                    if not isinstance(media_info, dict):
                        continue
                    filename = media_info.get("filename") or media_info.get("name")
                    ...
                    if not media_bytes:
                        ...
                        continue
                    item = encode_media_output(job_id, filename, media_bytes, errors)
                    if item:
            ...
            other_keys = [
                k
                for k in node_output.keys()
                if k not in {"images", "videos", "video", "gifs"}
            ]
            if other_keys:
                warn_msg = (
        ...
        # New ComfyUI versions may save PreviewVideo files without exposing a
        # media collection in history. In that case, collect files created by
        # this execution directly from the output directory.
        if not video_data:
            videos_after = snapshot_output_videos()
            changed_paths = [
                path
                for path, signature in videos_after.items()
                if videos_before.get(path) != signature
            ]
            for path in sorted(changed_paths, key=os.path.getmtime):
                try:
                    with open(path, "rb") as video_file:
                        media_bytes = video_file.read()
                    relative_name = os.path.relpath(path, COMFY_OUTPUT_DIR).replace(os.sep, "/")
                    item = encode_media_output(job_id, relative_name, media_bytes, errors)
                    if item:
                        video_data.append(item)
                except OSError as exc:
                    errors.append(f"Failed to read video output {path}: {exc}")
    ...
    print(
        f"worker-comfyui - Job completed. Returning {len(output_data)} image(s) "
        f"and {len(video_data)} video(s)."
    )
    return final_result
